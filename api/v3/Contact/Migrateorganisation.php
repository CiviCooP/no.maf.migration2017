 ...
 */
function _civicrm_api3_contact_migrateorganisation_spec(&$spec) {
}
 ...
 */
function civicrm_api3_contact_migrateorganisation($params) {
  set_time_limit(0);
  $returnValues = array();
  $entity = 'organisation';
  $createCount = 0;
  ...
  $daoSource = CRM_Core_DAO::executeQuery('SELECT * FROM migration_organisation WHERE is_processed = 0 ORDER BY master_id, id LIMIT %1', array(1=>array($limit, 'Integer')));
  while ($daoSource->fetch()) {
    $civiContact = new CRM_Migration_Organisation($entity, $daoSource, $logger);
    $newContact = $civiContact->migrate();
    if ($newContact == FALSE) {
      $logCount++;
      $updateQuery = 'UPDATE migration_organisation SET is_processed = %1 WHERE id = %2';
      CRM_Core_DAO::executeQuery($updateQuery, array(1 => array(1, 'Integer'), 2 => array($daoSource->id, 'Integer')));
    } else {
      $createCount++;
      $updateQuery = 'UPDATE migration_organisation SET is_processed = %1, new_contact_id = %2 WHERE id = %3';
  ...
  return civicrm_api3_create_success($returnValues, $params, 'Contact', 'Migrateorganisation');
}
