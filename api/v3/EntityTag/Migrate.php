...
function civicrm_api3_entity_tag_Migrate($params) {
  ...
  $daoSource = CRM_Core_DAO::executeQuery('SELECT * FROM migration_tags 
    WHERE is_processed = 0 ORDER BY id LIMIT %1', array(1=>array($limit, 'Integer')));
  while ($daoSource->fetch()) {
    $civiGroupContact = new CRM_Migration_EntityTag($entity, $daoSource, $logger);
    $success = $civiGroupContact->migrate();
    if ($success == FALSE) {
      $logCount++;
      $updateQuery = 'UPDATE migration_tags SET is_processed = %1 WHERE id = %2';
  ...
  if (empty($daoSource->N)) {
    ...
    $returnValues[] = $createCount.' entity tags migrated to CiviCRM, '.$logCount.' with logged errors that were not migrated';
  }
